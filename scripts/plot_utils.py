    ...
    for subset, configs in metrics.items():
        for config, config_dir in configs.items():
            ...
            metrics_df = pd.read_parquet(
                metrics_path, columns=metric_cols + groupby_columns[config]
            )
            metrics_df = (
                metrics_df.groupby(groupby_columns[config])[metric_cols]
                .mean()
                .reset_index()
            )
...
def plot_map_per_config(
    ...
    for i in range(n_subsets):
        ...
        kde_axes[i] = fig.add_subplot(
            gs[5, i], sharex=scatter_axes[i], sharey=None if i == 0 else kde_axes[0]
        )
...
def add_well_location(data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    well_list = [
        (num, lit_i + 1, f"{lit}{num:02}")
        for lit_i, lit in enumerate(list(string.ascii_uppercase)[15::-1])
